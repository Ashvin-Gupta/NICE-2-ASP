...
class RuleProcessor:
    ...
    def append_fired_rules(self, input_path: str, output_path: str) -> None:
        ...
        This method:
        - Processes ASP rules (lines containing :-) and adds fired() predicates
        - Comments out rule number markers (e.g., [1.1.4])
        - Comments out non-ASP lines (lines without :-)
        ...
        while i < len(lines):
            ...
            if line and (':-' in line or line.startswith(':-')):
                ...
                # Empty line or non-ASP line
                if not line:
                    # Preserve empty lines
                    if i < len(lines) - 1:
                        output_lines.append("")
                elif line:
                    # Non-ASP line (doesn't contain :-), comment it out
                    output_lines.append(f"% {line}")
                i += 1
